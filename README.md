...
### 方式三：搭配 Clash Party（mihomo-party）
Clash Party 内置 PAC 服务器会在运行时自动替换 `%mixed-port%`，与本仓库的占位符设计直接兼容：
1. **系统代理** → 代理模式选择 **PAC**（Clash Party 会在 `127.0.0.1:10000+` 启动 PAC 服务器并自动设置 Windows 代理）
2. 点击 **编辑 PAC 脚本**，粘贴 `cardo.pac` 全文并保存 —— 无需修改第一行，混合端口自动替换
3. 建议开启：**开机自启**（避免开机没网）、**内核设置 → 切换订阅时热重载配置**（避免订阅更新重启内核造成瞬时拒连）
4. 建议关闭：**自动进入轻量模式**（轻量模式会停止 PAC 服务器）、**TUN**（与系统代理互斥）
> ⚠️ `cardo.pac` 每周自动更新，更新后需重新粘贴到 PAC 编辑器。
